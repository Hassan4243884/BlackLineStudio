...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        blog_posts = list(filter(lambda x: x if x['type'] == 'post' else None, posts))
        attachments = list(filter(lambda x: x if x['type'] == 'attachment' else None, posts))
        attachments = {x['ID']: x for x in attachments}
        self.should_import_comments = options.get('import_comments')
        self.create_blog_pages(blog_posts, blog_index, attachments=attachments)
    ...
    def create_blog_pages(self, posts, blog_index, *args, attachments=None, **options):
        ...
        for post in posts:
            ...
            if seo_description:
                seo_description = self.convert_html_entities(seo_description)
            ...
            try:
                ...
                if seo_description:
                    new_entry.search_description = seo_description
                new_entry.save()
            except BlogPage.DoesNotExist:
                ...
                if seo_description:
                    new_entry.search_description = seo_description
            if post.get('thumbnail_id') and post.get('thumbnail_id') in attachments:
                header_image = None
                attachment = attachments.get(post.get('thumbnail_id'))
                remote_image = urllib.request.urlretrieve(
                    self.prepare_url(attachment['url']))
                try:
                    path, file_ = os.path.split(attachment['url'])
                    width = 640
                    height = 290
