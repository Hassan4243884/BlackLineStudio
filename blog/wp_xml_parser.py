...
class XML_parser(object):
        ...
        for e in item:
            if "category" in e.tag:
                ...
                cat_dict = self.category_dict.get(slug) or {"slug":slug,
                                                             "name":name,
                                                             "taxonomy":e.attrib.get('domain', 'category')}
            ...
            elif e.tag == '{wp}postmeta':
                if e.find('{wp}meta_key').text == '_yoast_wpseo_metadesc':
                    if e.find('{wp}meta_value') is not None:
                        ret_dict['seo_description'] = e.find('{wp}meta_value').text
                elif e.find('{wp}meta_key').text == '_thumbnail_id':
                    if e.find('{wp}meta_value') is not None:
                        ret_dict['thumbnail_id'] = e.find('{wp}meta_value').text
                elif e.find('{wp}meta_key').text == '_wp_attached_file':
                    if e.find('{wp}meta_value') is not None:
                        ret_dict['thumbnail_path'] = e.find('{wp}meta_value').text
    ...
    def convert_date(d, custom_date_string=None, fallback=None):
        ...
        '2015-03-30'
        """
        if d == 'Wed, 30 Nov -0001 00:00:00 +0000' and fallback:
            d = fallback
        try:
    ...
    def translate_item(self, item_dict):
        ...
        if not item_dict.get('title'):
            return None
        # Skip attachments
        if item_dict.get('{wp}post_type', None) == 'post':
            ret_dict = {'type': 'post'}
            ...
            ret_dict['thumbnail_id']= item_dict.get('thumbnail_id', None)
            ret_dict['seo_description']= item_dict['seo_description'] if 'seo_description' in item_dict else ''
            # fake user object
        ...
        elif item_dict.get('{wp}post_type', None) == 'attachment':
            ret_dict = {'type': 'attachment'}
            ret_dict['url'] = item_dict.get('{wp}attachment_url')
            ret_dict['ID'] = item_dict.get('{wp}post_id')
        else:
            return None
        return ret_dict
