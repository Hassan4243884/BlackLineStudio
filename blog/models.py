...
def get_blog_context(context):
    ...
    context['latest_posts'] = BlogPage.objects.live().order_by('-date')[:2]
    return context
...
class BlogIndexPage(Page):
    ...
    def get_context(self, request, tag=None, category=None, author=None, *args,
        ...
        if request.GET.get('q'):
            blogs = blogs.search(request.GET.get('q'))
        ...
        context['search_query'] = request.GET.get('q', None)
...
class BlogPage(Page):
    body = RichTextField(editor='tinymce', verbose_name=_('body'), blank=True)
